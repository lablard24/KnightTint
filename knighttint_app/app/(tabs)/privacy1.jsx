...
export default function Privacy() {
  ...
  // Ensure windowNumber is set to 1
  const currentWindowNumber = 1;
  ...
  const increaseTint = () => {
    if (ws.current) {
      ws.current.send(JSON.stringify({ window: currentWindowNumber, action: 'on', value: 100 }));
      setWindowData(prevData => ({
        ...prevData,
        [currentWindowNumber]: {
  ...
  const decreaseTint = () => {
    if (ws.current) {
      ws.current.send(JSON.stringify({ window: currentWindowNumber, action: 'off', value: 0 }));
      setWindowData(prevData => ({
        ...prevData,
        [currentWindowNumber]: {
          ...prevData[currentWindowNumber],
