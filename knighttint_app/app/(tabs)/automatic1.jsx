...
const Automatic = () => {
  ...
  const windowNumber = 1; // Adjust this if needed based on your application logic
  const ws = useRef(null);
  ...
  const updateTintLevel = (temperature, lux) => {
    //console.log('Updating tint level based on temperature:', temperature, 'and lux:', lux);
  
    let newTintLevel = 50;
  
    conditions.forEach((condition) => {
      if (condition.type === 'temperature' && temperature >= condition.temperatureValue) {
       // console.log(`Condition matched: ${temperature} >= ${condition.temperatureValue}`);
        newTintLevel = Math.max(newTintLevel, condition.tintLevel);
      } else if (condition.type === 'lux' && lux >= condition.luxValue) {
        console.log(`Condition matched: ${lux} >= ${condition.luxValue}`);
        newTintLevel = Math.max(newTintLevel, condition.tintLevel);
      }
    });
  
    //console.log('New tint level:', newTintLevel);
    setTintLevel(newTintLevel);
    sendTintLevelToWebSocket(newTintLevel);
  };
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.conditionTypeContainer}>
              <TouchableOpacity
                style={[styles.conditionTypeButton, conditionType === 'temperature' && styles.conditionTypeButtonActive]}
                onPress={() => setConditionType('temperature')}
              >
                <Text>Temperature</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.conditionTypeButton, conditionType === 'lux' && styles.conditionTypeButtonActive]}
                onPress={() => setConditionType('lux')}
              >
                <Text>Lux</Text>
              </TouchableOpacity>
            ...
            {conditionType === 'temperature' && (
              <TextInput
                ...
                value={temperatureCondition ? temperatureCondition.toString() : ''}
                onChangeText={(text) => setTemperatureCondition(parseFloat(text))}
                keyboardType="numeric"
              />
            )}
            {conditionType === 'lux' && (
              <TextInput
                ...
                placeholder="Lux"
                value={luxCondition ? luxCondition.toString() : ''}
                onChangeText={(text) => setLuxCondition(parseFloat(text))}
                keyboardType="numeric"
              />
            )}
            <TouchableOpacity style={styles.saveButton} onPress={saveCondition}>
