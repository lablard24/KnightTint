...
router.post('/login', async (req, res) => {
  try {
    ...
    const isPasswordValid = await bcrypt.compare(password, user.password); // Use bcryptjs.compare
...
router.post('/resetPassword', async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(newPassword, 10); // Use bcryptjs.hash
    user.password = hashedPassword;
    await user.save();
